...
module('Integration | Component | @frontile/forms-legacy/FormField::Radio', function (hooks) {
  setupRenderingTest(hooks);

  test('it renders with html attributes', async function (assert) {
    await render(<template><Radio name="some-name" data-test-radio /></template>);
  ...
  test('it renders @name arg', async function (assert) {
    await render(<template><Radio @name="my-name" data-test-radio /></template>);
  ...
  test('renders @checked arg, does not mutate it by default', async function (assert) {
    const value = cell(undefined);

    await render(
      <template><Radio data-test-radio @checked={{value.current}} @value="something" /></template>
    );

    assert.dom('[data-test-radio]').isNotChecked();
    await click('[data-test-radio]');

    assert.equal(value.current, undefined, 'should have not mutated the value');
  });
