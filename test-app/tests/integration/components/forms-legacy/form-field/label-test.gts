...
module('Integration | Component | @frontile/forms-legacy/FormField::Label', function (hooks) {
  ...
  test('it renders content, attributes and args', async function (assert) {
    ...
    assert.dom('[data-test-id="form-field-label"]').hasAttribute('for', 'some-input-id');

    assert.dom('[data-test-id="form-field-label"]').hasClass('form-field-label');
  ...
  test('it adds size classes for @size', async function (assert) {
    ...
    await render(<template><Label data-test-input @size={{size.current}} /></template>);
