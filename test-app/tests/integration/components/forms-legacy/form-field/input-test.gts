...
module('Integration | Component | @frontile/forms-legacy/FormField::Input', function (hooks) {
  setupRenderingTest(hooks);

  test('it renders with html attributes', async function (assert) {
    await render(<template><Input name="some-name" data-test-my-input /></template>);
  ...
  test('it should allow to change the input type', async function (assert) {
    await render(<template><Input @type="number" data-test-my-input /></template>);

    assert.dom('[data-test-my-input]').hasAttribute('type', 'number');
  });

  test('it renders @id arg', async function (assert) {
    await render(<template><Input @id="my-id" data-test-my-input /></template>);
  ...
  test('renders @value arg, does not mutate it by default', async function (assert) {
    ...
    await render(<template><Input class="my-input" @value={{value.current}} /></template>);
