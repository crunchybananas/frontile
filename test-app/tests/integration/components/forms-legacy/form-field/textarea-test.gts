...
module('Integration | Component | @frontile/forms-legacy/FormField::Textarea', function (hooks) {
  setupRenderingTest(hooks);

  test('it renders with html attributes', async function (assert) {
    await render(<template><Textarea name="some-name" data-test-textarea /></template>);
  ...
  test('it renders @id arg', async function (assert) {
    await render(<template><Textarea @id="my-id" data-test-textarea /></template>);
  ...
  test('renders @value arg, does not mutate it by default', async function (assert) {
    ...
    await render(<template><Textarea class="my-input" @value={{value.current}} /></template>);

    assert.dom('.my-input').hasValue('Josemar');

    await fillIn('.my-input', 'Sam');

    assert.equal(value.current, 'Josemar', 'should have not mutated the value');
  });
  ...
  test('should call @onInput function arg', async function (assert) {
    ...
    await render(
      <template>
        <Textarea class="my-input" @value={{value.current}} @onInput={{setName}} />
      </template>
    );
  ...
  test('should call @onChange function arg', async function (assert) {
    ...
    await render(
      <template>
        <Textarea class="my-input" @value={{value.current}} @onChange={{setName}} />
      </template>
    );
