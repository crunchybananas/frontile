...
module('Integration | Component | Popover | @frontile/overlays', function (hooks) {
  ...
  test('it works with trigger and opening content', async function (assert) {
    ...
    assert
      ...
      .hasAttribute('data-test-id', 'content', 'should have focused in the content');
  });
  ...
  test('clicking outside closes menu', async function (assert) {
    ...
    await render(
      <template>
        ...
        <Popover @didClose={{didClose}} as |p|>
          ...
          <p.Content @backdrop="none" @disableTransitions={{true}} data-test-id="content">
            Content here
          </p.Content>
        </Popover>
      </template>
    ...
    assert
      .dom(document.activeElement)
      .hasAttribute('data-test-id', 'trigger', 'should have restored the focus to the triggeer');
  });
