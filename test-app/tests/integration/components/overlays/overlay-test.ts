...
import { render, settled, click, triggerKeyEvent, find } from '@ember/test-helpers';
import hbs from 'htmlbars-inline-precompile';
import { registerCustomStyles } from '@frontile/theme';
import { tv } from 'tailwind-variants';

module('Integration | Component | @frontile/overlays/Overlay', function (hooks) {
  ...
  test('it renders the content, into portal and only when opened', async function (assert) {
    ...
    assert.dom('[data-test-id="overlay"]').hasText('My Content Something focusable');
  ...
  test('when @renderInPlace={{true}} renders in place', async function (assert) {
    ...
    await render(template);
    assert.dom('[data-portal-target] > [data-test-id="overlay"]').doesNotExist();
    // @ts-ignore
  ...
  test('it manages focusing in content and restoration when focusTrap is disabled', async function (assert) {
    ...
    assert
      ...
      .hasAttribute('data-test-id', 'overlay', 'should have focused in the overlay');
    ...
    assert
      .dom(document.activeElement)
      .hasAttribute('data-test-id', 'some-button', 'should have restored the focus');
    ...
    assert
      .dom(document.activeElement)
      .doesNotHaveAttribute('data-test-id', 'should have not restored the focus');
  });
});
