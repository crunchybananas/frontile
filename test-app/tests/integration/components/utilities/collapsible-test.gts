...
module('Integration | Component | @frontile/utilities/Collapsible', function (hooks) {
  ...
  test('renders content and starts open', async function (assert) {
    ...
    assert.dom('[data-test-id=collapsible]').hasText('Content');
    assert.dom('[data-test-id=collapsible]').doesNotHaveStyle({ height: '0px' });
  ...
  test('renders initial height when set', async function (assert) {
    ...
    await render(
      <template>
        <Collapsible @isOpen={{isOpen.current}} @initialHeight="2px" data-test-id="collapsible">
          Content
        </Collapsible>
      </template>
    );
    ...
    assert.dom('[data-test-id=collapsible]').hasStyle({ overflow: 'visible' });
    assert.dom('[data-test-id=collapsible]').hasText('Content');
    assert.dom('[data-test-id=collapsible]').doesNotHaveStyle({ height: '2px' });
