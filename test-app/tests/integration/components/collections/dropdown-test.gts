...
module('Integration | Component | Dropdown | @frontile/collections', function (hooks) {
    ...
    await render(
      <template>
        <Dropdown as |d|>
          ...
          <d.Menu @onAction={{onAction}} @intent="primary" @disableTransitions={{true}} as |Item|>
  ...
  test('it renders accessibility attributes', async function (assert) {
    ...
    assert.dom('[data-test-id="dropdown-trigger"]').hasAria('haspopup', 'true');
    assert.dom('[data-test-id="dropdown-trigger"]').hasAria('expanded', 'false');
    assert.dom('[data-test-id="dropdown-trigger"]').hasAttribute('aria-controls');

    await click('[data-test-id="dropdown-trigger"]');

    assert.dom('[data-test-id="dropdown-trigger"]').hasAria('expanded', 'true');
  });
    ...
    await render(
      <template>
        <Dropdown as |d|>
          ...
          <d.Menu @backdrop={{backdrop.current}} @disableTransitions={{true}} as |Item|>
            <Item @key="profile">My Profile</Item>
            <Item @key="settings">Settings</Item>
          </d.Menu>
        </Dropdown>
  ...
  test('on pressing arrow up/down key, opens the menu', async function (assert) {
    ...
    assert.dom('[data-test-id="listbox"]').doesNotExist();
    await triggerKeyEvent('[data-test-id="dropdown-trigger"]', 'keyup', 'ArrowDown');
