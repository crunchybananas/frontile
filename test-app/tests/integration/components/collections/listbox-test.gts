...
module('Integration | Component | Listbox | @frontile/collections', function (hooks) {
  ...
  test('it render static items', async function (assert) {
    ...
    assert.dom('[data-key="item-1"] [data-test-id="listbox-item-description"]').exists();

    assert.dom('[data-key="item-2"]').exists();
    assert.dom('[data-key="item-2"] [data-test-id="listbox-item-shortcut"]').exists();
  ...
  test('it render dynamic items without yield of item selectionMode = single / multiple', async function (assert) {
    ...
    assert.dom('[data-key="crocodile"]').hasAttribute('data-selected', 'true');
    ...
    assert.equal(selectedKeys.current[0], 'elephant');
    assert.dom('[data-key="elephant"]').hasAttribute('data-selected', 'true');
    assert.dom('[data-key="crocodile"]').hasAttribute('data-selected', 'false');
  ...
  test('keyboard navigation works', async function (assert) {
    ...
    assert
      ...
      .hasAttribute('data-active', 'true', 'should have selected crocodile due to two keypress');
    assert.dom('[data-key="elephant"]').hasAttribute('data-active', 'false');
  });
  ...
  test('it render item with blocks', async function (assert) {
    ...
    assert.dom('[data-test-id="listbox"]').exists();
    assert.dom('[data-key="item-1"]').exists();
    assert.dom('[data-key="item-1"] [data-test-id="listbox-item-label"]').exists();
    assert.dom('[data-key="item-1"] [data-test-id="listbox-item-label"]').hasText('Item 1');

    assert.dom('[data-key="item-1"] [data-test-id="start"]').exists();
    assert.dom('[data-key="item-1"] [data-test-id="start"]').hasText('Start content');
    assert.dom('[data-key="item-1"] [data-test-id="end"]').exists();
    assert.dom('[data-key="item-1"] [data-test-id="end"]').hasText('End content');
  });
  ...
  test('automatically activate first item', async function (assert) {
    ...
    await render(
      <template><Listbox @items={{animals.current}} @autoActivateMode="first" /></template>
    );
    assert.dom('[data-key="cheetah"]').hasAttribute('data-active', 'true');
  ...
  module('style classes', () => {
    test('it adds class for default appearance', async function (assert) {
      ...
      const appearance = cell<ListboxSignature<unknown>['Args']['appearance']>();
