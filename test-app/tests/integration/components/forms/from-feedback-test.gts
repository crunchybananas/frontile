...
import { FormFeedback, type FormFeedbackSignature } from '@frontile/forms';
import { cell } from 'ember-resources';

module('Integration | Component | @frontile/forms/FormFeedback', function (hooks) {
  ...
  hooks.beforeEach(async function () {
    await render(
      <template>
        <FormFeedback @id="feedback" @messages={{messages.current}} @intent={{intent.current}} />
      </template>
    );
  });
  ...
  test('it renders a list of messages', async function (assert) {
    ...
    assert.dom('[data-component="form-feedback"]').hasAttribute('id', 'feedback');
  });
  ...
  test('it renders aria-live', async function (assert) {
    ...
    assert.dom('[data-component="form-feedback"]').hasAria('live', 'assertive');
